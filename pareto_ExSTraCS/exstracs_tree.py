...
#         self.sumPreFitnessEC = 1.0
    ...
    def setPhenotype(self, args): #Ensure that for every instance we recalculate the tree phenotype prediction.  It never stays fixed. 
        ...
        if dataInfo.discretePhenotype and len(dataInfo.phenotypeList) == 2:
            ...
            if not self.epochComplete: #Not sure where Ben came up with this but it seems to makes reasonable sense.  May want to examie more carefully. 
                self.phenotype_RP = ((cons.env.formatData.classProportions['1'] * self.one_count) + (cons.env.formatData.classProportions['0'] * self.zero_count)) / (self.one_count + self.zero_count)
...
#         print self.phenotype
        while ref < len(cons.env.formatData.phenotypeRanked) and cons.env.formatData.phenotypeRanked[ref] <= phenRange[1]:
        ...
        for idx, val in enumerate(rule.specifiedAttList):
            if val not in tree_args:
                if random.choice([0, 1]) == 0:
                    rule_cross.append(rule.specifiedAttList.pop(idx)) #what is idx here?  should be the position in the list (may be an error here)
