...
class ExSTraCS:
    ...
    def runIteration(self, state_phenotype):
        ...
        if phenotypePrediction == None or phenotypePrediction == 'Tie':
            if cons.env.formatData.discretePhenotype:
                ...
                phenotypePrediction = random.uniform(cons.env.formatData.phenotypeList[0],cons.env.formatData.phenotypeList[1])
        else:
        #-------------------------------------------------------
        ...
        #-----------------------------------------------------------------------------------------------------------------------------------------
        self.population.runGA(self.exploreIter, state_phenotype[0], state_phenotype[1]) #GA is run within the correct set.
        #-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    def doContPopEvaluation(self, isTrain):
        ...
        adjustedAccuracyEstimate = (accuracyEstimate * instanceCoverage) + ((1.0 - instanceCoverage) * (1.0 / 3.0)) #1/3 is expected random error within range 0 to 1 with uniform selection.
