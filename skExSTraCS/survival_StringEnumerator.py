...
class StringEnumerator:
    def __init__(self, inputFile, survivalTime, censoringIndicator):
        ...
        #----------------------------------------------
        tempEventArray = np.empty(len(self.dataEventTimes),dtype=object)
        for instanceIndex in range(len(self.dataEventTimes)):
            tempEventArray[instanceIndex] = str(self.dataEventTimes[instanceIndex])
        self.dataEventTimes = tempEventArray
        
        tempStatusArray = np.empty(len(self.dataEventStatus),dtype=object)
        for instanceIndex in range(len(self.dataEventStatus)):
            tempStatusArray[instanceIndex] = str(self.dataEventStatus[instanceIndex])
        self.dataEventStatus = tempStatusArray

        tempFeatureArray = np.empty((len(self.dataEventTimes),len(self.dataHeaders)),dtype=object)
...
#--------------------------------------------------------------------------------------------------------------------       
   def print_invalid_attributes(self):
        ...
        distinctValues = []
        isInvalid = False
        for instIndex in range(len(self.dataEventTimes)):
            val = self.dataEventTimes[instIndex]
            if not val in distinctValues and val != "NA":
                distinctValues.append(self.dataEventTimes[instIndex])
            if val != "NA":
                try:
                    float(val)
                except:
                    isInvalid = True
        if isInvalid:
            print(str(self.classLabel)+" (the EventTime): ",end="")
            for i in distinctValues:
                print(str(i)+"\t",end="")
            print()
            
            ###could add here another chunk to identify invalid eventStatus values
#--------------------------------------------------------------------------------------------------------------------
# change_class_name: lets user change the class label if it is the same as one of the attribute labels
#--------------------------------------------------------------------------------------------------------------------   
...
#--------------------------------------------------------------------------------------------------------------------
# add_attribute_converter: Convers attributes to XXX
#-------------------------------------------------------------------------------------------------------------------- 
