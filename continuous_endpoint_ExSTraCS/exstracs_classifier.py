...
class Classifier:
    def __init__(self,a=None,b=None,c=None,d=None):
        ...
        self.accuracy = 1.0             # Classifier accuracy - Accuracy calculated using only instances in the dataset which this rule matched.
        ...
        self.errorSum = 0
        self.errorCount = 0
        self.origin = None   #temporary code to track down the birth origin of a given classifier
    ...
    def uniformCrossover(self, cl, state, phenotype):
        ...
        if cons.env.formatData.discretePhenotype or random.random() < 0.5: #Always crossover condition if the phenotype is discrete (if continuous phenotype, half the time phenotype crossover is performed instead)
            self.origin += '_CrossoverC' #temporary code 
            cl.origin += '_CrossoverC' #temporary code 
    ...
    def mutateContinuousAttributes(self, useAT, j):
        ...
        if useAT:
            if random.random() < cons.AT.getTrackProb()[j]: #High AT probability leads to higher chance of mutation (Dives ExSTraCS to explore new continuous ranges for important attributes)
                ...
                i = self.specifiedAttList.index(j) #reference to the position of the attribute in the rule representation
                attRange = self.condition[i][1] - self.condition[i][0]
        ...
        if self.condition[i][0] > self.condition[i][1]:
...
#                print trueMax
                ...
                if self.condition[i][0] <= trueMin and self.condition[i][1] >= trueMax: # Rule range encloses entire training range
                    ...
                    self.condition[i][1] = trueMax + valBuffer
                else:
                    pass  
    ...
    def getDelProp(self, meanFitness):
        ...
        if self.fitness/self.numerosity >= cons.delta*meanFitness or (self.matchCount < cons.theta_del and not self.epochComplete):
    ...
    def printClassifier(self):
        ...
        classifierString += str(self.deletionVote)+"\t"+str(self.correctCount)+"\t"+str(self.matchCount)+"\t"+str(self.correctCover)+"\t"+str(self.matchCover)+"\t"+str(epoch)+"\t"+str(self.origin)+"\n"
